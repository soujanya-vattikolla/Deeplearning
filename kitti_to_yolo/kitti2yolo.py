...
def resolveClazzNumberOrNone(clazz, use_dont_care):
    ...
    #if clazz in (KEY_TRUCK):
    if clazz == KEY_TRUCK:
        return CLAZZ_NUMBERS[KEY_TRUCK]
    #if clazz in (KEY_DONT_CARE):
...
def parseArguments():
    ...
    parser.add_argument("image_2_dir")
    parser.add_argument("--training-samples", type=int, default=0.8)
